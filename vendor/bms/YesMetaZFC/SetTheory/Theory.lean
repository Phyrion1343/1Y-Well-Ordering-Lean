...
def SatisfiesSentence (ℳ : SetTheory.Structure.{v}) (sentence : Definitional.Project.Sentence) : Prop :=
  Definitional.Structure.SatisfiesSentence
    Definitional.Project.kernel ℳ sentence
/-- 项目句子语义通过项目公式的按需解释器读取。 -/
theorem satisfiesSentence_iff (ℳ : SetTheory.Structure.{v}) (sentence : Definitional.Project.Sentence) :
...
def SemanticallyEntails (theory : Theory) (target : Definitional.Project.Sentence) : Prop :=
...
theorem entails_of_mem {theory : Theory}
    {target : Definitional.Project.Sentence} (hTarget : theory target) :
    SemanticallyEntails.{v} theory target := by
  intro ℳ hModels
  exact hModels.2 target hTarget
theorem entails_weaken {strong weak : Theory}
    {target : Definitional.Project.Sentence} (hSubset : ∀ sentence, weak sentence → strong sentence) (hEntails : SemanticallyEntails.{v} weak target) :
